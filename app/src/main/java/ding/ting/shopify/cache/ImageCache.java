...
import ding.ting.shopify.util.VersionUtil;
...
public class ImageCache {
     ...
     */
    public boolean addBitmapToCache(String data, Bitmap bitmap) {
        if (data == null || bitmap == null) {
            return false;
        }

        // Add to memory cache
        if (mMemoryCache != null && mMemoryCache.get(data) == null) {// mMemoryCache设置为false了，此处为null不会占用内存空间
    ...
    public File getFileFromDiskCache(String data) {
        ...
        synchronized (mDiskCacheLock) {
            ...
            if (mDiskLruCache != null) {
                try {
                    final File[] files = mDiskLruCache.getFile(key);
                    if (files == null) {
                        return null;
                    }
                    File file = files[DISK_CACHE_INDEX];
                    return file;
                } catch (final IOException e) {
                }
            }
    ...
    public static class ImageCacheParams {
         ...
         * its constructor. This value should be chosen carefully based on a
         * number of factors Refer to the corresponding Android Training class
         * for more discussion:
    ...
    public static File getExternalCacheDir(Context context) {
        if (VersionUtil.hasFroyo()) {
            return context.getExternalCacheDir() == null ? context.getCacheDir() : context
                    .getExternalCacheDir();
        }
